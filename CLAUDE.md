...
- **Planner model is `openai/gpt-oss-120b`, a reasoning model.** `reasoning_format` (used on Qwen/DeepSeek) does **not** apply to `gpt-oss` models on Groq — use `extra_body={"reasoning_effort": "low", "include_reasoning": False}` instead, and pass it via `extra_body` since these aren't in the `openai` SDK's typed `.create()` signature. Without this, reasoning tokens can eat the whole `max_tokens` budget and return empty content (bit `gpt-oss-20b` once, then `llama-3.3-70b-versatile` after Groq deprecated it 2026-06-17 — see `notes/system-b-history.md`, 2026-08-17 entry). Keep `max_tokens` well under this Groq org's 8000 TPM on-demand-tier cap on `gpt-oss-120b` (input + max_tokens both count). Before swapping models again, check `console.groq.com/docs/deprecations` — Groq's per-model docs pages stay live after a model is decommissioned, so a page existing doesn't mean the model is still callable.
