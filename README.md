...
Groq deprecated `llama-3.3-70b-versatile` for free/dev-tier accounts on 2026-06-17; the 404 landed once the grace period ended (hit 2026-08-16). Fixed by swapping to `openai/gpt-oss-120b`. If you see this again, check `console.groq.com/docs/deprecations` before picking a replacement model — the docs pages for deprecated models often stay live and can mislead you into thinking a decommissioned model is still active.

### `Completions.create() got an unexpected keyword argument 'reasoning_format'` (historical)

Hit immediately after the fix above. `reasoning_format` isn't in the `openai` SDK's typed `.create()` signature — it's a Groq-specific field that must go through `extra_body`, not passed as a direct kwarg. Separately, `reasoning_format` turned out to not even apply to `gpt-oss` models on Groq (Qwen/DeepSeek only) — the correct params for `gpt-oss` are `reasoning_effort` (`"low"`, actually reduces reasoning-token spend) and `include_reasoning` (`False`). Also hit a `413 tokens_per_minute` rate limit at `max_tokens=8192` — this Groq org's free/on-demand tier caps `gpt-oss-120b` at 8000 TPM total (input + max_tokens), so `max_tokens` was brought back down to 4096. Verified working end-to-end via `scripts/test_plan.py` after all three fixes landed together.
...
**Planner — `core/llm_client.py`** calls **`openai/gpt-oss-120b`** on Groq with `response_format={"type":"json_object"}` (`json_schema` is not used — the explicit JSON skeleton in `SYSTEM_PROMPT` plus Pydantic validation carries that weight instead) and `extra_body={"reasoning_effort":"low","include_reasoning":False}` (this is a reasoning model — `reasoning_format`, which works on Qwen/DeepSeek, is **not supported** on gpt-oss models; `reasoning_effort` is what actually caps reasoning-token consumption here, and without it hidden reasoning can consume the whole `max_tokens` budget and leave nothing for the actual JSON output, the failure mode that bit `gpt-oss-20b`, see Troubleshooting). Both are Groq-specific params outside the `openai` SDK's typed signature, so they must go through `extra_body` — passing them as direct kwargs raises `TypeError: unexpected keyword argument`. `max_tokens=4096`, kept low deliberately: this Groq org is on the free/on-demand tier with an 8000 TPM cap on `gpt-oss-120b`, and `max_tokens` counts against that alongside the input. The response is parsed and validated against a Pydantic `WeeklyPlan` model before being returned.
