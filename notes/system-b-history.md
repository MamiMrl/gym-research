...
- First deploy attempt broke immediately with `Completions.create() got an unexpected keyword argument 'reasoning_format'` — `reasoning_format` isn't in the `openai` SDK's typed signature and needed `extra_body` instead. Turned out `reasoning_format` doesn't apply to `gpt-oss` models on Groq at all (Qwen/DeepSeek only) — the real fix is `extra_body={"reasoning_effort": "low", "include_reasoning": False}`.
- That surfaced a third issue: `413 tokens_per_minute` at `max_tokens=8192` — this Groq org is on the free/on-demand tier, capped at 8000 TPM on `gpt-oss-120b` (input + `max_tokens` together). Brought `max_tokens` back down to 4096.
- Verified working end-to-end against the live Groq API via `scripts/test_plan.py` only after all three fixes landed together — each earlier attempt looked plausible but failed on the next call.

**Lesson:** Every currently-live text model on Groq (`gpt-oss-120b`, `gpt-oss-20b`, `qwen3.6-27b`, `minimax-m2.7`) is a reasoning model now — there's no more "boring non-reasoning model" escape hatch on this provider. Reasoning-token control is now a permanent requirement, not a one-off fix — but the exact param differs by model family (`reasoning_effort`/`include_reasoning` for `gpt-oss`, `reasoning_format` for Qwen/DeepSeek), and none of it is in the `openai` SDK's typed `.create()` signature, so it always needs `extra_body`. Don't trust a fix here until it's been run against the live API, not just compiled — three different failure modes surfaced only when actually called.
